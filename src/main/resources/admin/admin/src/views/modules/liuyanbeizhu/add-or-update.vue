<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="留言内容" prop="liuyanneirong">
          <el-input v-model="ruleForm.liuyanneirong" 
              placeholder="留言内容" clearable  :readonly="ro.liuyanneirong"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="留言内容" prop="liuyanneirong">
              <el-input v-model="ruleForm.liuyanneirong" 
                placeholder="留言内容" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="留言时间" prop="liuyanshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.liuyanshijian" 
                type="datetime"
                :readonly="ro.liuyanshijian"
                placeholder="留言时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.liuyanshijian" label="留言时间" prop="liuyanshijian">
...
export default {
  ...
  created() {
	this.ruleForm.liuyanshijian = this.getCurDateTime()
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `liuyanbeizhu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.liuyanbeizhuCrossAddOrUpdateFlag = false;
						 this.parent.search();
